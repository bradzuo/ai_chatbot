...
class Interpreter:
    ...
    @staticmethod
    def default_output_attributes(sender_id) -> Dict[Text, Any]:
        return {"intent": {"name": None, "confidence": 0.0}, "entities": [],"sender_id":sender_id}
    ...
    def parse(
        ...
        if not text:
            ...
            output = self.default_output_attributes(sender_id)
            output["text"] = ""
            return output
        # print('--nlu_model:',self.default_output_attributes(sender_id))
        message = Message(text, self.default_output_attributes(sender_id), time=time)
